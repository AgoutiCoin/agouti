...
// 6161 = 1*5000 + 1*1000 + 1*500 + 1*100 + 1*50 + 1*10 + 1*5 + 1
static const int ZQ_6161 = 6161;
...
enum AvailableCoinsType {
    ...
    ONLY_DENOMINATED = 2,
    ONLY_NOT10000IFMN = 3,
    ONLY_NONDENOMINATED_NOT10000IFMN = 4, // ONLY_NONDENOMINATED and not 10000 AGU at the same time
    ONLY_10000 = 5,                        // find masternode outputs including locked ones (use with caution)
    STAKABLE_COINS = 6                          // UTXO's that are valid for staking
};

// Possible states for zAGU send
enum ZerocoinSpendStatus {
    ZAGU_SPEND_OKAY = 0,                            // No error
    ZAGU_SPEND_ERROR = 1,                           // Unspecified class of errors, more details are (hopefully) in the returning text
    ZAGU_WALLET_LOCKED = 2,                         // Wallet was locked
    ZAGU_COMMIT_FAILED = 3,                         // Commit failed, reset status
    ZAGU_ERASE_SPENDS_FAILED = 4,                   // Erasing spends during reset failed
    ZAGU_ERASE_NEW_MINTS_FAILED = 5,                // Erasing new mints during reset failed
    ZAGU_TRX_FUNDS_PROBLEMS = 6,                    // Everything related to available funds
    ZAGU_TRX_CREATE = 7,                            // Everything related to create the transaction
    ZAGU_TRX_CHANGE = 8,                            // Everything related to transaction change
    ZAGU_TXMINT_GENERAL = 9,                        // General errors in MintToTxIn
    ZAGU_INVALID_COIN = 10,                         // Selected mint coin is not valid
    ZAGU_FAILED_ACCUMULATOR_INITIALIZATION = 11,    // Failed to initialize witness
    ZAGU_INVALID_WITNESS = 12,                      // Spend coin transaction did not verify
    ZAGU_BAD_SERIALIZATION = 13,                    // Transaction verification failed
    ZAGU_SPENT_USED_ZAGU = 14                       // Coin has already been spend
};
...
public:
    ...
    /** Find the most recent valid StakePointer for this masternode wallet.
     *  Returns true and fills stakePointerOut if an eligible, unused pointer is found.
     *  Returns false if this node is not eligible to stake (no valid MN reward in window). */
    bool GetStakePointer(StakePointer& stakePointerOut) const;
    ...
    */
    ...
    void setZAgoutiAutoBackups(bool fEnabled)
    {

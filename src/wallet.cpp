...
bool CWallet::GetBudgetSystemCollateralTX(CWalletTx& tx, uint256 hash, bool useIX)
{
    ...
    vecSend.push_back(make_pair(scriptChange, GetBudgetProposalFee(chainActive.Height())));
...
bool CWallet::CreateCoinStake(const CKeyStore& keystore, unsigned int nBits, int64_t nSearchInterval, CMutableTransaction& txNew, unsigned int& nTxNewTime)
{
    ...
    BOOST_FOREACH (const PAIRTYPE(uint256, unsigned int)& stakeRef, setStakeCoins) {
        ...
        if (CheckStakeKernelHash(nBits, block, *pcoin.first, prevoutStake, nTxNewTime, nHashDrift, false, hashProofOfStake, true, chainActive.Height() + 1)) {
            ...
            // Found a valid stake input (kernel or stake-pointer mode).
        ...
        LogPrintf("CommitTransaction:\n%s", wtxNew.ToString());
        {
            ...
            if (!wtxNew.IsZerocoinSpend()) {
                ...
                BOOST_FOREACH (const CTxIn& txin, wtxNew.vin) {
                    ...
                    auto mit = mapWallet.find(txin.prevout.hash);
                    if (mit == mapWallet.end()) continue;
                    mit->second.BindWallet(this);
        ...
        if (pcoin->vin.size() > 0) {
            ...
            BOOST_FOREACH (CTxIn txin, pcoin->vin) {
                ...
                if (!IsMine(txin)) /* If this input isn't mine, ignore it */
                    continue;
                auto wit = mapWallet.find(txin.prevout.hash);
                if (wit == mapWallet.end() || txin.prevout.n >= wit->second.vout.size())
                    continue;
                if (!ExtractDestination(wit->second.vout[txin.prevout.n].scriptPubKey, address))
                    continue;
...
{
    receipt.SetStatus("Zerocoin is disabled", ZAGU_TXMINT_GENERAL);
    return false;
}
...
{
    receipt.SetStatus("Zerocoin is disabled", ZAGU_TRX_FUNDS_PROBLEMS);
    return false;
}

string CWallet::ResetMintZerocoin(bool fExtendedSearch)
{
    return _("Zerocoin is disabled");
}
...
{
    receipt.SetStatus("Zerocoin is disabled", ZAGU_SPEND_ERROR);
    return false;
}

...
std::atomic<int64_t> nReserveBalance{0};
...
bool GetCoinAge(const CTransaction& tx, const unsigned int nTxTime, uint64_t& nCoinAge)
{
    ...
    LogPrint("stakepointer", "coin age bnCoinDay=%s\n", bnCoinDay.ToString().c_str());
...
bool AcceptToMemoryPool(CTxMemPool& pool, CValidationState& state, const CTransaction& tx, bool fLimitFree, bool* pfMissingInputs, bool fRejectInsaneFee, bool ignoreFees)
{
    ...
    // Reject zerocoin transactions — no new mints or spends accepted
    if (tx.ContainsZerocoins())
        return state.DoS(100, error("AcceptToMemoryPool: : zerocoin transactions are permanently disabled"),
            REJECT_INVALID, "bad-tx-zerocoin");
...
bool AcceptableInputs(CTxMemPool& pool, CValidationState& state, const CTransaction& tx, bool fLimitFree, bool* pfMissingInputs, bool fRejectInsaneFee, bool isDSTX)
{
    ...
    // Reject zerocoin transactions — no new mints or spends accepted
    if (tx.ContainsZerocoins())
        return state.DoS(100, error("AcceptableInputs: : zerocoin transactions are permanently disabled"),
            REJECT_INVALID, "bad-tx-zerocoin");
...
bool RecalculateAGUSupply(int nHeightStart)
{
...
bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck)
{
    ...
    // BIP66 (DERSIG) — buried activation, enforced from genesis on this chain
    ...
    for (unsigned int i = 0; i < block.vtx.size(); i++) {
        ...
        // Reject zerocoin after the cutoff height
        if (pindex->nHeight >= 2565000 && tx.ContainsZerocoins()) {
            return state.DoS(100, error("ConnectBlock() : zerocoin transactions are permanently disabled"));
        }

        if (tx.IsZerocoinSpend()) {
            // Historical zerocoin spend — calculate value from nSequence (denomination)
            for (const CTxIn& txIn : tx.vin) {
    ...
    int nCalcHeight = (pindex->nHeight >= Params().StakePointerForkHeight()) ? pindex->nHeight : pindex->pprev->nHeight;
...
void static UpdateTip(CBlockIndex* pindexNew)
{
    ...
    // If turned on AutoZeromint will automatically convert AGU to zAGU
...
bool CheckBlock(const CBlock& block, CValidationState& state, bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig)
{
    ...
    if (block.GetBlockTime() > GetAdjustedTime() + (block.IsProofOfStake() ? 180 : 7200)) // 3 min loose bound; tighter 60s enforced by height in ContextualCheckBlockHeader
    ...
    if (IsSporkActive(SPORK_3_SWIFTTX_BLOCK_FILTERING)) {
        ...
        LogPrint("more", "CheckBlock() : skipping transaction locking checks\n");
    }
    ...
    if (pindexPrev != NULL) {
        ...
        if (nHeight != 0 && !IsInitialBlockDownload()) {
            ...
            LogPrint("more", "CheckBlock(): Masternode payment check skipped on sync - skipping IsBlockPayeeValid()\n");
        }
    }
